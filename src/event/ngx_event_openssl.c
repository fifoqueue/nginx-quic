...
static int
ngx_ssl_client_hello_get0_received_ext(const SSL_CLIENT_HELLO *client_hello,
    uint16_t *exts, size_t *num_exts)
{
    ...
    if (client_hello == NULL || num_exts == NULL) {
        return 0;
    }

    p = client_hello->extensions;
    last = p + client_hello->extensions_len;
    n = 0;
    ...
    while (p < last) {
        ...
        if (exts != NULL) {
            exts[n] = (uint16_t) ((p[0] << 8) | p[1]);
        }

        len = (uint16_t) ((p[2] << 8) | p[3]);
        p += 4;
...
ngx_ssl_client_hello_ja4_cb(const SSL_CLIENT_HELLO *client_hello)
{
    ngx_connection_t  *c;
    ngx_str_t         *ja;
    ...
    ngx_ssl_client_hello_get0_received_ext(client_hello, NULL, &num_exts);
    exts_len = num_exts * sizeof(uint16_t);

    ciphers = client_hello->cipher_suites;
    ciphers_len = client_hello->cipher_suites_len;
    log_ssl_ext(c, "ciphers", ciphers, ciphers_len);

    SSL_early_callback_ctx_extension_get(client_hello,
                                         TLSEXT_TYPE_supported_groups, &groups,
                                         &groups_len);
    SSL_early_callback_ctx_extension_get(client_hello,
                                         TLSEXT_TYPE_ec_point_formats, &formats,
                                         &formats_len);
    ...
    /* version */
    legacy_version = client_hello->version;
    ...
    ngx_ssl_client_hello_get0_received_ext(client_hello, exts, &num_exts);
    ...
    *ptr++ = (u_char) (SSL_is_quic(client_hello->ssl)
                       ? 'q'
                       : SSL_is_dtls(client_hello->ssl) ? 'd' : 't');
#else
    *ptr++ = (u_char) (SSL_is_dtls(client_hello->ssl) ? 'd' : 't');
#endif
...
ngx_ssl_handshake(ngx_connection_t *c)
{
    ...
    if (ngx_ssl_is_setting_client_hello_ja4_callback) {
        SSL_CTX_set_select_certificate_cb(c->ssl->session_ctx,
                                          ngx_ssl_client_hello_ja4_cb);
    }
